...
namespace faif {
    namespace search {
        ...
        template<typename Ind> struct EvolutionaryAlgorithmSpace : public Space<Ind> {
            ...
            typedef std::vector<std::pair<Ind, double> > Population2;
        };
        ...
        template<typename Space> struct CrossoverCustom {
            ...
            /** \brief calls the mutation function from Space */
            static typename Space::Individual& crossover(typename Space::Individual& ind, const typename Space::Population2& pop, int empty) {
                return Space::crossover(ind, pop);
            }
        };
        ...
        template<typename Space> struct SelectionRanking {
            ...
            */
            ...
            static typename Space::Population2& selection(typename Space::Population2& population, int size) {
                typename Space::Population2::iterator middle = population.begin();
                std::advance(middle, size);
                typedef typename Space::Individual Individual;
                typedef typename std::pair<Individual, double> pair;

                //the n-th best elements is moved to the beginning of contener
                std::nth_element(population.begin(), middle, population.end(),
                                 boost::bind(&pair::second, _1) > boost::bind(&pair::second, _2) );
        ...
        */
        ...
        protected:
            ...
            /** \brief No action for MutationNone. */
            void doMutation(Population2&, TransformationNoneTag) { /* do nothing */ }
            /** \brief Called when MutationCustom is given. */
            void doMutation(Population2& population, TransformationCustomTag) {
                typename Population2::iterator it;
                for (it = population.begin(); it != population.end(); ++it) {
                	&Mutation<Space>::mutation(it->first);
                }
            ...
            void doCrossover(Population2&, const Population2&, TransformationNoneTag) { /* do nothing */ }
            void doCrossover(Population2& population, const Population2& wholePop, TransformationCustomTag) {
                typename Population2::iterator it;
                for (it = population.begin(); it != population.end(); ++it) {
                	 &Crossover<Space>::crossover(it->first, wholePop, 1);
                }
            }
        };
